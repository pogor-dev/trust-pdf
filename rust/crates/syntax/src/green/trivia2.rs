...
impl<'a> GreenNodeTrait<'a, u64> for GreenTrivia2<'a> {
    #[inline]
...
impl hash::Hash for GreenTrivia2<'_> {
...
impl PartialEq for GreenTrivia2<'_> {
    fn eq(&self, other: &Self) -> bool {
        self.kind == other.kind && self.width == other.width && self.text == other.text
    }
}

impl Eq for GreenTrivia2<'_> {}

unsafe impl Send for GreenTrivia2<'_> {}
unsafe impl Sync for GreenTrivia2<'_> {}

impl fmt::Debug for GreenTrivia2<'_> {
...
mod tests {
    ...
    fn test_kind(#[case] kind: SyntaxKind) {
        let token = GreenTrivia2::new_with_text(kind, b"some text".into());
    ...
    fn test_to_string(#[case] kind: SyntaxKind, #[case] text: &[u8]) {
        let token = GreenTrivia2::new_with_text(kind, text.into());
    ...
    #[rstest]
    fn test_is_trivia() {
        let token = GreenTrivia2::new_with_text(SyntaxKind::WhitespaceTrivia, b" ".into());
    ...
    fn test_eq(#[case] kind: SyntaxKind, #[case] expected_kind: SyntaxKind, #[case] text: &[u8], #[case] expected_text: &[u8], #[case] expected: bool) {
        let token = GreenTrivia2::new_with_text(kind, text.into());
        let other = GreenTrivia2::new_with_text(expected_kind, expected_text.into());
