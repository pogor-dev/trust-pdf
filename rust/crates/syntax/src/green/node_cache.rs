...
use crate::{
    GreenNode, GreenNodeData, GreenToken, GreenTokenData, GreenTrivia, NodeOrToken, SyntaxKind,
...
fn token_hash(token: &GreenTokenData) -> u64 {
    ...
    token.leading_trivia().text().hash(&mut h);
    token.trailing_trivia().text().hash(&mut h);
    h.finish()
}
...
impl NodeCache {
    ...
    pub(crate) fn token(
        ...
        let hash = {
            ...
            kind.hash(&mut h);
            text.hash(&mut h);
            leading.text().hash(&mut h);
            trailing.text().hash(&mut h);
            h.finish()
        };
        ...
        let entry = self.tokens.raw_entry_mut().from_hash(hash, |token| {
            token.0.kind() == kind
                && token.0.text() == text
                && token.0.leading_trivia().text() == leading.text()
                && token.0.trailing_trivia().text() == trailing.text()
        });
