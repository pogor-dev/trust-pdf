...
use crate::{
    Direction, Language, NodeOrToken, SyntaxElement, SyntaxText, SyntaxToken, WalkEvent,
    api::{Preorder, PreorderWithTokens},
    green::{GreenNode, GreenNodeData},
    red,
};

/// Represents a node in the syntax tree for a given language.
#[derive(Clone, PartialEq, Eq, Hash)]
pub struct SyntaxNode<L: Language> {
    pub(super) raw: red::SyntaxNode,
...
impl<L: Language> SyntaxNode<L> {
    ...
    pub fn first_child_by_kind(&self, matcher: &impl Fn(L::Kind) -> bool) -> Option<SyntaxNode<L>> {
        self.raw.first_child_by_kind(&|raw_kind| matcher(L::kind_from_raw(raw_kind))).map(Self::from)
    }
    ...
    pub fn next_sibling_by_kind(&self, matcher: &impl Fn(L::Kind) -> bool) -> Option<SyntaxNode<L>> {
        self.raw.next_sibling_by_kind(&|raw_kind| matcher(L::kind_from_raw(raw_kind))).map(Self::from)
    }
    ...
    // pub fn token_at_offset(&self, offset: u64) -> TokenAtOffset<SyntaxToken<L>> {
    ...
    pub fn covering_element(&self, range: Range<u64>) -> SyntaxElement<L> {
    ...
    pub fn child_or_token_at_range(&self, range: Range<u64>) -> Option<SyntaxElement<L>> {
    ...
    //     let to_insert = to_insert.into_iter().map(red::SyntaxElement::from);
