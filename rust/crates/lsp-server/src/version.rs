//! Code for representing TRust PDF Analyzer's release version number.

use std::fmt;

/// Information about the git repository where TRust PDF Analyzer was built from.
...
pub struct VersionInfo {
    /// TRust PDF Analyzer's version, such as "1.57.0", "1.58.0-beta.1", "1.59.0-nightly", etc.
...
pub const fn version() -> VersionInfo {
    ...
    let commit_info = match (
        option_env!("TRUST_PDF_COMMIT_SHORT_HASH"),
        option_env!("TRUST_PDF_COMMIT_HASH"),
        option_env!("TRUST_PDF_COMMIT_DATE"),
    ) {
